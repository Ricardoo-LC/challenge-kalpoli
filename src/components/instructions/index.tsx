...
export const Instructions = ({ show, close }: Props) => {
  ...
  return (
    <div
      className={`fixed flex justify-center items-start z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full bg-black bg-opacity-50 transition-transform transition-opacity duration-500 ${
        show ? 'modal' : 'modal-closed'
      }`}
    >
      <div className="relative min-h-max w-full max-w-xl sm:rounded-2xl bg-slate-50 border border-neutral-950 text-black font-medium dark:bg-[#262B3C] dark:border dark:border-slate-400 dark:text-white">
        <div className="my-10 mx-10">
          ...
          <p className="whitespace-normal pt-4 font-bold">Ejemplos</p>
          <div className="pt-4 grid grid-cols-5 gap-5">
            {LETTER_CORRECT.split('').map((letter, index) => (
              <Letter
                key={index}
                value={letter}
                status={letter === 'G' ? 'correct' : 'instructions'}
              />
            ))}
          </div>
          ...
          <div className="pt-3 grid grid-cols-5 gap-5">
            {LETTER_PRESENT.split('').map((letter, index) => (
              <Letter
                key={index}
                value={letter}
                status={letter === 'C' ? 'present' : 'instructions'}
              />
            ))}
          </div>
          <p className="whitespace-normal pt-3 ">
          ...
          <div className="pt-3 grid grid-cols-5 gap-5">
            {LETTER_ABSENT.split('').map((letter, index) => (
              <Letter
                key={index}
                value={letter}
                status={letter === 'O' ? 'absent' : 'instructions'}
              />
            ))}
          </div>
          <p className="whitespace-normal pt-3">
